...
class MerchantTokoDDSSupabaseUser extends BaseAuthUser {
  MerchantTokoDDSSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> merchantTokoDDSSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MerchantTokoDDSSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
